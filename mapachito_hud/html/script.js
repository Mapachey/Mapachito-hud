$(function(){
    ...
    window.addEventListener("message", function(event){
        ...
        if (data.whenUse) {
            if (Math.round(data.o2) > 99) {
                $("#o2").hide(300)
            } else if (Math.round(data.o2) < 99) {
                $("#o2").show(300)
            }
            if (Math.round(data.stamine) > 100) {
                $("#stamine").hide(300)
            } else if (Math.round(data.o2) < 100) {
                $("#stamine").show(300)
            }
            if (Math.round(data.health) > 100) {
                $("#health").hide(300)
            } else if (Math.round(data.health) < 100) {
                $("#health").show(300)
            }
            if (Math.round(data.shield) > 25) {
                $("#shield").show(300)
            } else if (Math.round(data.shield) < 25) {
                $("#shield").hide(300)
            }
            if (Math.round(data.hunger) > 100) {
                $("#hunger").hide(300)
            } else if (Math.round(data.hunger) < 100) {
                $("#hunger").show(300)
            }
            if (Math.round(data.thirst) > 100) {
                $("#thirst").hide(300)
            } else if (Math.round(data.thirst) < 100) {
                $("#thirst").show(300)
            }
        }

        $("#o2Bar").css({"height": Math.round(data.o2) + "%"});
        $("#stamineBar").css({"height": Math.round(data.stamine) + "%"});
